...
import hypaad
from hypaad.algorithms.algorithm_executor import AlgorithmRuntimeException
from hypaad.trial_result import TrialResult
...
class Optimizer:
    ...
    def run_and_score_algorithm(
        ...
        results_base_dir = Path("/tmp/hypaad-anomaly-scores")
    ...
    def _create_objective(self) -> t.Callable[[optuna.Trial], float]:
        ...
        def func(trial: optuna.Trial):
            ...
            trial_result: hypaad.TrialResult = None
            if len(self.timeseries_names) == 1:
                ...
                with worker_client() as client:
                    trial_group: t.List[hypaad.TrialResult] = (
                ...
                trial_result = hypaad.TrialResult(
                    study_name=self.study_name,
                    optuna_study_name=self.optuna_study_name,
                    id=trial.number,
                    worker=get_worker().name,
        ...
        try:
            optuna_study.optimize(
                ...
                n_trials=n_trials,
                    ...
                    hypaad.AlgorithmRuntimeException,
