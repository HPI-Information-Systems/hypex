        ...
        with hypaad.Cluster(self.cluster_config) as cluster:
            ...
            for study in studies:
                ...
                if study.start_from_snapshot is not None:
                    ...
                    results_data_generation[study.name] = hypaad.DataGenerationModule(
                        seed=self.seed
                    ).run(
                        cluster=cluster,
            ...
            for study in studies:
                if (
                    ...
                    name = f"{study.start_from_snapshot.study_name}.{study.timeseries.name}"
                    results_fixed_parameters[study.name] = hypaad.Validator.Result.load(
                        study_name=name,
                        base_dir=results_dir,
                    )
                else:
            ...
            for study in studies:
                ...
                if (
                    ...
                    name = f"{study.start_from_snapshot.study_name}.{study.timeseries.name}"
                    results_evaluation[study.name] = hypaad.Evaluator.Result.load(
                        study_name=name,
                        base_dir=results_dir,
                    )
                else:
